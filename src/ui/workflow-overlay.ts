...
export class WorkflowOverlay implements Component {
  ...
  private renderProgress(width: number, _height?: number): string[] {
    ...
    const rendered = this.renderNodeList(
      nodes,
      innerWidth,
      maxNodeVisible,
    );
    ...
    for (let ni = 0; ni < nodes.length; ni++) {
      ...
      if (
        ...
        for (const iter of node.iterations) {
          allLines.push(
            this.renderIterationLine(iter, innerWidth, th),
          );
          lineNodeIdx.push(ni);
        }
      }
  ...
  private renderNodeLine(
    ...
    let toolBadge = "";
    if (isRunning) {
      const activeTc = node.toolCalls.find(
        (tc) => tc.output === undefined,
      );
      if (activeTc) {
        const tcElapsed = Math.floor(
          (Date.now() - activeTc.startedAt) / 1000,
        );
        toolBadge = th.fg(
          "accent",
  ...
  private renderIterationLine(
    iter: { iteration: number; state: string; duration?: number; error?: string },
    innerWidth: number,
    th: Theme,
  ): string {
  ...
  private renderOutputPanel(
    ...
    if (this.outputAutoFollow) {
      const maxOutScroll = Math.max(
        0,
        contentLines.length - maxOutVisible,
      );
    ...
    if (contentLines.length === 0) {
      ...
      if (this.outputScrollOffset + maxOutVisible < contentLines.length) {
        ...
        lines[lines.length - 1] = th.fg(
          "dim",
          `↓ ${moreBelow} lines below`,
        );
      }
    }
   ...
   * Merges streaming AI text, tool call cards, and fallback log lines
   * into a unified list of rendered strings.
   */
  private buildOutputContent(
    ...
    // Priority 1: If we have streaming text + tool calls from conversation SSE
    if (node.streamingText || node.toolCalls.length > 0) {
      // Interleave streaming text with tool call cards
      // Strategy: split streamingText at tool call insertion points
      // For simplicity, show: streaming text block, then tool call cards
      ...
      for (const tc of node.toolCalls) {
        contentLines.push(
          ...this.renderToolCallCard(tc, innerWidth, th),
        );
      }
    ...
    if (!isCompleted) {
      ...
      if (tc.expanded && tc.output) {
        ...
        if (outLines.length > 8) {
          lines.push(
            th.fg("dim", `  … ${outLines.length - 8} more lines`),
          );
        }
      } else {
        ...
        lines.push(
          th.fg("success", `▸ ${tc.name}`) +
            th.fg("dim", ` (${durStr})`) +
            (outputPreview
              ? th.fg("dim", ` ${truncateToWidth(outputPreview, innerWidth - 20)}`)
              : ""),
        );
      }
    }
  ...
  private renderLogs(width: number, height?: number): string[] {
    ...
    // Build unified content: streaming text + tool call cards + fallback
    const contentLines: string[] = [];
    ...
    if (node.streamingText || node.toolCalls.length > 0) {
      ...
      for (const line of outLines) {
        contentLines.push(
          this.renderLogLine(line, innerWidth - 4, th),
        );
      }
    } else {
      // Fallback log lines
    ...
    const maxLogVisible = height
      ? Math.max(height - 6, 5)
      : 15;
    const maxLogScroll = Math.max(
      0,
      contentLines.length - maxLogVisible,
    );
    ...
    if (contentLines.length === 0) {
      ...
      if (
        this.logScrollOffset + maxLogVisible <
        contentLines.length
      ) {
        const moreBelow =
          contentLines.length -
  ...
  private handleLogsInput(data: string): boolean {
    ...
    if (data === "x") {
      if (this.inspectedNode) {
        for (
          let i = this.inspectedNode.toolCalls.length - 1;
          i >= 0;
          i--
        ) {
          if (this.inspectedNode.toolCalls[i].output !== undefined) {
            this.tracker.toggleToolCallExpanded(
              this.inspectedNode.id,
              i,
            );
            return true;
          }
