...
export async function handleToolCommand(
	...
	if (cmd === "help")
		return formatToolTextResult(generateHelpForPath(argTokens));

	const helpArgs = argTokens.filter(
		(token) => !["help", "-h", "--help"].includes(token.toLowerCase()),
	);
	if (helpArgs.length !== argTokens.length) {
		return formatToolTextResult(
			generateHelpForPath(cmd.split(":").concat(helpArgs)),
		);
	}
	...
	const toolCtx: ToolCommandContext = {
		...
		hasUI: false,
		ui: {
			custom: async <T>(_renderer: TuiRendererFn): Promise<T | undefined> =>
				undefined,
	...
	if (result.handler && result.rest.length === 0) {
		...
		return {
			...output,
			details: { delegated: result.path, ...(output.details ?? {}) },
		};
	}
	if (result.meta) {
		return formatToolTextResult(
			generateHelpForPath(cmd.split(":").concat(argTokens)),
		);
	}
	return formatToolTextResult(
		formatArchonMessage(`- **Unknown:** \`${safeCode(cmd)}\``),
	);
}

export async function handleCliFallback(
	pi: ExtensionAPI,
