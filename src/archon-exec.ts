...
import { formatElapsed, hasFlag } from "./helpers";
...
function resolveInvocation(
	...
	const cmdArgs = hasFlag(args, "--cwd")
		? [...args]
		: [...args, "--cwd", projectCwd];
	if (fs.existsSync(`${ARCHON_ROOT}/package.json`)) {
		return {
			command: "bun",
			args: ["run", "cli", ...cmdArgs],
			cwd: ARCHON_ROOT,
		};
	}
...
async function execCore(
	...
	if (!opts?.onLine && (opts?.buffered ?? true) && pi) {
		const result = await pi.exec(inv.command, inv.args, {
			cwd: inv.cwd,
...
export async function runArchonCommandStreaming(
	...
	return execCore(undefined, args, projectCwd, {
		signal,
		onLine,
		buffered: false,
	});
...
export async function runArchonCommandWithToolUpdates(
	...
	const interval = setInterval(() => {
		pushUpdate(
			"running",
			`Archon ${label} running (${formatElapsed(Math.floor((Date.now() - startedAt) / 1000))})...`,
		);
	}, PROGRESS_UPDATE_MS);
...
export function formatArchonOutput(
	title: string,
	run: ArchonRunResult,
	durationMs?: number,
	input?: string,
...
export function formatArchonToolResult(
	...
	return {
		content: [
			{
				type: "text" as const,
				text: formatArchonOutput(title, run, durationMs),
			},
		],
		details: {
			...details,
