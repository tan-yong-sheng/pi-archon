...
async function handleRun(
  ...
  ctx?: ToolCtx,
): Promise<{ content: { type: string; text: string }[]; details: Record<string, unknown>; terminate?: boolean }> {
  if (!workflow) {
    ...
    if (runId) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Workflow **${workflow}** launched in background (run ID: ${runId}). ` +
  ...
  const { runArchonCommand, formatArchonOutput } = await import("./archon-exec");
  ...
  try {
    const { findLatestRunId, queryRunArtifacts: queryArts } = await import("./artifact-query");
  ...
  const cleaned = formatArchonOutput(
    ...
    result,
    durationMs,
  ).split("\n").filter((l: string) => !/^\[(?:INF|WRN)\] /m.test(l)).join("\n");
...
async function handleStatus(
  ...
  if (runId) {
        ...
        content: [
          {
            ...
            text: [
              ...
              tracker.workflowError ? `- **Error:** ${tracker.workflowError}` : "",
              "",
              "### Nodes",
              "",
              ...nodes.map(
    ...
    if (dbRun) {
      ...
      let artifacts: unknown[] = [];
      try {
        const { queryRunArtifacts, queryRunNodeSummaries: queryNodes } = await import("./artifact-query");
            ...
            text: [
              ...
              ...(artifacts.length > 0
                ? [
                    "### Artifacts",
                    "",
                    ...(artifacts as { label: string; type: string; url?: string }[]).map(
                      (a) => `- ${a.type}: ${a.label}${a.url ? ` — ${a.url}` : ""}`,
                    ),
                    "",
                  ]
                : []),
  ...
  if (recentRuns.length > 0) {
    ...
    for (const run of recentRuns.slice(0, 5)) {
      ...
      lines.push(`- ${icon} \`${run.id.slice(0, 8)}\` **${run.workflowName}**${dur} — ${run.status}`);
    }
  }
